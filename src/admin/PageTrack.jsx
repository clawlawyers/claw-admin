...
const PageTrack = () => {
  ...
  const fetchPageData = useCallback(async () => {
    try {
      ...
      const res = await getAllVisitors(queryParams);
      ...
      if (res && res.data && Array.isArray(res.data)) {
        ...
        res.data.forEach((visit) => {
          ...
          pageVisits[path].visits += 1;

          // Add duration
          pageVisits[path].totalDuration += visit.visitDuration || 0;

          // Track unique visitors
          if (visit.visitorId) {
            pageVisits[path].uniqueVisitors.add(visit.visitorId);
          }

          // Track unique users
          if (visit.userId) {
            pageVisits[path].uniqueUsers.add(visit.userId);
          }

          // Update last visit time
          const visitTime = dayjs(visit.timestamp);
          if (
            !pageVisits[path].lastVisit ||
            visitTime.isAfter(pageVisits[path].lastVisit)
          ) {
            pageVisits[path].lastVisit = visitTime;
          }
        });

        // Convert to array and calculate averages
        const pagesArray = Object.values(pageVisits).map((page) => ({
          ...page,
          uniqueVisitors: page.uniqueVisitors.size,
          uniqueUsers: page.uniqueUsers.size,
          avgDuration:
            page.visits > 0 ? Math.round(page.totalDuration / page.visits) : 0,
          lastVisit: page.lastVisit
            ? page.lastVisit.format("YYYY-MM-DD HH:mm")
            : "N/A",
        }));
                ...
                <tbody className="text-white">
                  {sortedData.length === 0 ? (
                    ...
                    sortedData.map((page, index) => (
                      ...
                      >
                        <td className="p-3 text-left">{page.path}</td>
